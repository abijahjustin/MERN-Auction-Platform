...
export const addNewAuctionItem=catchAsyncErrors(async(req,res,next)=>{
    ...
    if(alreadyOneAuctionActive.length>0){
        return next(new ErrorHandler("You alreay have one auction active.",400));
    }
    try {
...
export const republishItem=catchAsyncErrors(async(req,res,next)=>{
    ...
    if(!req.body.startTime || !req.body.endTime){
        return next(new ErrorHandler("Startime and Endtime for republish is mandatory."))
    }
    ...
    if(data.startTime<Date.now()){
        return next(new ErrorHandler("Auction starting time must be greater than presemnt time",400));
    }
